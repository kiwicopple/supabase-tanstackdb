...
import { supabase, todosCollectionOptions, where, type Todo } from '@/lib/supabase'
...
export default function Home() {
  ...
  // Load todos based on filter and search
  const loadTodos = useCallback(async () => {
    setLoading(true)
    setError(null)

    try {
      // Build the load subset options based on filter and search
      ...
      if (filter === 'active') {
        conditions.push(where.eq('completed', false))
      } else if (filter === 'completed') {
        conditions.push(where.eq('completed', true))
      }

      // Add search condition (using ilike for case-insensitive search)
      if (searchQuery.trim()) {
